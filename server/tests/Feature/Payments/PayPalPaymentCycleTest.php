...
it('marks the payment as failed when paypal returns a terminal capture error', function () {
    app()->instance(PayPalService::class, new class(app(\App\Services\SettingsService::class)) extends PayPalService
    {
        public function createOrder(User $user, Course $course, string $successUrl, string $cancelUrl): array
        {
            return ['id' => 'order_terminal_failure', 'approve_url' => $successUrl];
        }
    ...
    $this->actingAs($student)
        ->postJson(route('payments.paypal.capture'), ['order_id' => 'order_terminal_failure'])
        ->assertStatus(422)
        ->assertJson([
            'ok' => false,
            'reason' => 'capture_incomplete',
    ...
    $payment = Payment::query()->where('external_reference', 'order_terminal_failure')->first();
